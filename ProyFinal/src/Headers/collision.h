...
bool testRayPlane(glm::vec3 orig, glm::vec3 dest, Plane p, float &t, glm::vec3 q)
{
	glm::vec3 ab = dest - orig;
	t = (p.Dir - glm::dot(p.Nor, orig)) / glm::dot(p.Nor, ab);
...
int testPlane(glm::vec3 a, glm::vec3 b, glm::vec3 d, glm::vec3 e, glm::vec3 f, Plane p, float &t, glm::vec3 &q)
{	
	p.Nor = glm::cross(e - d, f - d);
	p.Dir = glm::dot(p.Nor, d);
	return testRayPlane(a, b, p, t, q);
}
